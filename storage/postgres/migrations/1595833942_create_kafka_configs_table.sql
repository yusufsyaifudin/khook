...
-- INSERT INTO kafka_configs(id, name, namespace, rev, spec, status, created_at, updated_at) (SELECT x, concat('kafka-', x::text), 'default', 1, concat('{"apiVersion":"khook/v1","kind":"KafkaBrokerConnection","namespace":"default","name":"',  x::text, '","spec":{"brokers":["localhost:9092"]}}')::jsonb, 'start', extract(epoch from now()), extract(epoch from now()) FROM generate_series(1,100) AS x);
